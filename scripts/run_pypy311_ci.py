#!/usr/bin/env python3
"""Run kg-doc-parser's provider-free PyPy 3.11 compatibility profile."""
...
def main() -> int:
    ...
    if args.venv is not None and os.environ.get("KG_DOC_PARSER_PYPY_VENV_ACTIVE") != "1":
        venv = args.venv.resolve()
        if not venv.exists():
            subprocess.check_call([sys.executable, "-m", "venv", str(venv)], cwd=root())
        env = os.environ.copy()
        env["KG_DOC_PARSER_PYPY_VENV_ACTIVE"] = "1"
        return subprocess.call(
            [str(venv_python(venv)), str(Path(__file__).resolve()), "--skip-install"],
            cwd=root(),
            env=env,
        )
