[CmdletBinding()]
param(
    [string]$Image = "kg-doc-parser-pypy311-ci:local",
    [switch]$SkipBuild
)
...
docker run --rm -i `
    --env KG_DOC_PARSER_PYPY=1 `
    --volume "${root}:/workspace" `
    --workdir /workspace `
    --entrypoint pypy3 $Image `
    scripts/run_pypy311_ci.py --venv /tmp/pypy311-venv
if ($LASTEXITCODE -ne 0) { throw "kg-doc-parser PyPy 3.11 Linux CI profile failed with exit code $LASTEXITCODE" }
